...
export const getAllMessagesWithConversationId = CatchAsyncError(async (req, res) => {
    try {
        ...
        const messages = await Message.find({ conversation_id: new mongoose.Types.ObjectId(conversationId) }).sort({ createdAt: 1 }).populate({path:"sender_id", select:"name avatar role"});
